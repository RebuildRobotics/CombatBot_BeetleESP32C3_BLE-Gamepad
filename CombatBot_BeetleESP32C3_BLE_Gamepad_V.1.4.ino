...
// Channel values
int axelRY; // Drive channel: Forward/Backward drive speed
int axelRX; // Drive channel: Left/Right turn speed
// AI
bool AIActive = false;
// Weapon
...
int angPrev = -1;
long angleSetTime;
// If outside the range // https://bluepad32.readthedocs.io/en/latest/FAQ/
long lastUpdate;
// Signal safety
...
void setup() {
  ...
  // Bluepad bluetooth allowlist
  (allowList) ? uni_bt_allowlist_set_enabled(true) : uni_bt_allowlist_set_enabled(false);
  /* // Comment line above and uncomment lines below if having problems and need to reset and disable bluetooth allowlist
...
void loop() {
  ...
  // Run AI
  if (deviceConnected && AIActive && !safetyActive && !batteryEmpty) runAI();
}
// ---------------------- Functions --------------------
void EEPROM_Save() {
...
void onConnectedController(ControllerPtr ctl) { // This callback gets called any time a new gamepad is connected. // Up to 4 gamepads can be connected at the same time.
  ...
  if (!foundEmptySlot && DEBUG) Serial.println("Controller connected, but could not found empty slot");
...
void onDisconnectedController(ControllerPtr ctl) {
  ...
  for (int i = 0; i < BP32_MAX_GAMEPADS; i++) {
    if (myControllers[i] == ctl) {
      if (DEBUG) Serial.printf("Controller disconnected from index=%d\n", i);
  ...
  if (!foundController) {    
    if (DEBUG) Serial.println("Controller disconnected, but not found in myControllers");
  }
...
void monitorVoltage() {
  ...
  if (voltageLevel <= lowVoltageLimit) {
    ...
    if (batteryEmpty) {
      batteryEmpty = false;
      if (useLowVoltageGuard && deviceConnected) failsafe(false);
    }
  }
...
void processGamepad(ControllerPtr ctl) {
  // Battery low alarm
  if (!safetyActive && sendAlarm) ctl->playDualRumble(0, 500, 0x80, 0x40);
  // Signal safety
  if (ctl->r1()) ctl->playDualRumble(0, 300, 0x00, 0x40);
  if (ctl->r1() && !safetyActive) { // Signal safety On
    stopMotors();
    if (USE_ESC) setWeaponAngle(SRV_ANG_IDLE);
    setAI(false);
    resetChannels();
    safetyActive = true;
  } else if (ctl->r1() && safetyActive) {safetyActive = false;} // Signal safety Off
  // Programming mode
  if (safetyActive) {
    // Channel mix on/off
    if (ctl->y() && ctl->b()) (CHMIX) ? CHMIX = false : CHMIX = true;
    // Bluetooth allowList, add address to list
    if (ctl->y() && ctl->a()) {
      uni_bt_allowlist_add_addr(ctl->getProperties().btaddr);
      ctl->playDualRumble(0, 200, 0x80, 0x40);
    }
    // Bluetooth allowList, enable / disable + clear
    if (ctl->y() && ctl->miscButtons() == 4) { // Y + menu
      if (allowList) { // Clear and deactivate list
        uni_bt_allowlist_remove_all();
        allowList = false;
      } else {allowList = true;} // Activate list
      uni_bt_allowlist_set_enabled(allowList);
      ctl->playDualRumble(0, 200, 0x80, 0x40);
    }
  }
  // Allow other functions if signal safety is off and battery is not empty
  if (!safetyActive && !batteryEmpty) {
    // AI
    if (ctl->a() && !AIActive) setAI(true); // AI On
    else if (ctl->a() && AIActive) setAI(false); // AI Off
    // Weapon On/Off button
    if (ctl->l1()) ctl->playDualRumble(0, 100, 0x80, 0x00);
    if (ctl->l1() && !weaponActive) { // Wep On/Full
      if (!USE_BIDIR) setWeaponAngle(SRV_ANG_MAX);
    } else if (ctl->l1() && weaponActive) {setWeaponAngle(SRV_ANG_IDLE);} // Wep Off
    // Weapon stick (Left joystick)
    if (ctl->axisY()) {
      // Convert weapon signal to servo position
      int srvAng = ctl->axisY();
      if (invWepStick) srvAng = -srvAng;
    ...
    // Weapon pad
    static int weaponPadStep = 18; // (10 steps from 0 to 180)
    if (ctl->dpad() == 1 || ctl->dpad() == 2) {
      if (
    ...
    if (ctl->dpad() == 1) { // Up // Increase in steps of 18
      if (angPrev == -1) { // Calibrate if no other weapon control haven't been used before pressing pad
        setWeaponAngle(SRV_ANG_IDLE);
      } else { // Set angle
        if (SRV_ANG_IDLE < SRV_ANG_MAX) {
          if (SRV_ANG_MAX - angPrev >= weaponPadStep) setWeaponAngle(angPrev + weaponPadStep);
          else if (SRV_ANG_MAX - angPrev < weaponPadStep && SRV_ANG_MAX - angPrev > 0) setWeaponAngle(SRV_ANG_MAX);
        } else {
          if (abs(SRV_ANG_MAX - angPrev) >= weaponPadStep) setWeaponAngle(angPrev - weaponPadStep);
          else if (abs(SRV_ANG_MAX - angPrev) < weaponPadStep && abs(SRV_ANG_MAX - angPrev) > 0) setWeaponAngle(SRV_ANG_MAX);
        }
      }
    } else if (ctl->dpad() == 2) { // Down // Decrease in steps of 18
      if (angPrev == -1) { // Calibrate if no other weapon control haven't been used before pressing pad
        setWeaponAngle(SRV_ANG_IDLE);
      } else { // Set angle
        if (SRV_ANG_IDLE < SRV_ANG_MAX) {
          if ((USE_BIDIR && SRV_ANG_MIN + angPrev >= weaponPadStep) || (!USE_BIDIR && SRV_ANG_IDLE + angPrev >= weaponPadStep)) setWeaponAngle(angPrev - weaponPadStep);
          else if (USE_BIDIR && SRV_ANG_MIN + angPrev < weaponPadStep && SRV_ANG_MIN + angPrev > SRV_ANG_MIN) setWeaponAngle(SRV_ANG_MIN);
          else if (!USE_BIDIR && SRV_ANG_IDLE + angPrev < weaponPadStep && SRV_ANG_IDLE + angPrev > SRV_ANG_IDLE) setWeaponAngle(SRV_ANG_IDLE);
        } else {
          if ((USE_BIDIR && SRV_ANG_MIN - angPrev >= weaponPadStep) || (!USE_BIDIR && SRV_ANG_IDLE - angPrev >= weaponPadStep)) setWeaponAngle(angPrev + weaponPadStep);
          else if (USE_BIDIR && SRV_ANG_MIN - angPrev < weaponPadStep && SRV_ANG_MIN + angPrev > SRV_ANG_MIN) setWeaponAngle(SRV_ANG_MIN);
          else if (!USE_BIDIR && SRV_ANG_IDLE - angPrev < weaponPadStep && SRV_ANG_IDLE + angPrev > SRV_ANG_IDLE) setWeaponAngle(SRV_ANG_IDLE);
    ...
    if (ctl->y() && (ctl->dpad() == 8 || ctl->dpad() == 4) && TRIM_L > 0 && TRIM_L < 100 && TRIM_R > 0 && TRIM_R < 100) ctl->playDualRumble(0, 100, 0x80, 0x00);
    if (ctl->y() && ctl->dpad() == 8) { // Left button
      if (TRIM_L > 0 && TRIM_R == 100) {
        if (TRIM_L >= trimPadStep) TRIM_L -= trimPadStep; // Decrease left motor
        else TRIM_L -= TRIM_L;
      } else if (TRIM_R < 100 && TRIM_L == 100) {
        if (100-TRIM_R >= trimPadStep) TRIM_R += trimPadStep; // Increase right motor
        else TRIM_R += (100-TRIM_R);
      }
    }
    if (ctl->y() && ctl->dpad() == 4) { // Right button
      if (TRIM_R > 0 && TRIM_L == 100) {
        if (TRIM_R >= trimPadStep) TRIM_R -= trimPadStep; // Decrease right motor
        else TRIM_R -= TRIM_R;
      } else if (TRIM_L < 100 && TRIM_R == 100) {
        if (100-TRIM_L >= trimPadStep) TRIM_L += trimPadStep; // Increase left motor
        else TRIM_L += (100-TRIM_L);
      }
    }
    // Invert drive
    if (ctl->x() && !invert) invert = true; // Invert on
    else if (ctl->x() && invert) invert = false; // Invert off
    ...
    if (CHMIX) {
      throttleLeft = -axelRY + axelRX; // Original: y + x
      throttleRight = -axelRY - axelRX; // Original: y - x
    } else { // No channel mixing
      if (abs(axelRY) >= abs(axelRX)) { // Forward/backward
        throttleLeft = -axelRY;
        throttleRight = -axelRY;
      } else { // Full turns
        throttleLeft = axelRX;
        throttleRight = -axelRX;
      }
    }
    // Constrain 
    ...
    if (myController && myController->isConnected()) {
      if (myController->hasData()) {
        if (myController->isGamepad()) {
          ...
          if (DEBUG) errors += "Unsupported controller\n";
        }
      } else {
